...
const int hpix[] = { 50, 73, 45, 59, 54, 47, 59, 16, 16, 52, 66, 59, 75, 52, 65, 34, 43, 29, 29 };
...
int updateButtonStates() {
  ...
  for (int i = 0; i < 12; i++) {
    if (currentMillis - lastMillis > debounceTime) {
      if ((i <= 7) && buttonStates[i] && !lastButtonStates[i]) {
...
void drawVowel(int index) {
  tft.drawBitmap(67 - int(hpix[index] / 2), 160 - vpix[index], epd_bitmap_allArray[index], hpix[index], vpix[index], ST77XX_WHITE);
}

void drawAccent(int acindex, int vwindex) {
  if (acindex == 17) {
    tft.drawBitmap(59 - int(hpix[acindex] / 2), 155 - vpix[acindex] - vpix[vwindex], epd_bitmap_allArray[acindex], hpix[acindex], vpix[acindex], 0xfae0);
    return;
  }
  if (acindex == 18) {
    tft.drawBitmap(75 - int(hpix[acindex] / 2), 155 - vpix[acindex] - vpix[vwindex], epd_bitmap_allArray[acindex], hpix[acindex], vpix[acindex], 0xfae0);
    return;
  }
  tft.drawBitmap(67 - int(hpix[acindex] / 2), 155 - vpix[acindex] - vpix[vwindex], epd_bitmap_allArray[acindex], hpix[acindex], vpix[acindex], 0xfae0);
}

void drawSpecial(int index) {
  if (index == 15) {
    tft.drawBitmap(90 - int(hpix[index] / 2), 100 - vpix[index], epd_bitmap_allArray[index], hpix[index], vpix[index], 0x349f);
    return;
  }
  tft.drawBitmap(67 - int(hpix[index] / 2), 160 - vpix[index], epd_bitmap_allArray[index], hpix[index], vpix[index], 0x349f);
}

void typeAscii(int code) {
  bleKeyboard.press(KEY_LEFT_ALT);
  bleKeyboard.write(KEY_NUM_0);
  bleKeyboard.write(numKeys[code / 100]);
  bleKeyboard.write(numKeys[(code % 100) / 10]);
  bleKeyboard.write(numKeys[(code % 100) % 10]);
  bleKeyboard.releaseAll();
}

void updateFormatIndicator() {
  if (lastFormat != isHTML) {
    tft.fillRect(0, 0, 135, 40, ST77XX_BLACK);
    if (isHTML) {
      tft.drawBitmap(17, 5, epd_bitmap_HTML, 100, 27, 0x67e9);
      tft.drawRoundRect(12, 0, 110, 37, 4, ST77XX_YELLOW);
    } else {
      tft.drawBitmap(23, 5, epd_bitmap_ASCII, 88, 28, 0x67e9);
      tft.drawRoundRect(18, 0, 98, 38, 4, ST77XX_YELLOW);
    }
  }
  lastFormat = isHTML;
}

void updatePreview(int flagState) {
